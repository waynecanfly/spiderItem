...
class CompanylistSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for temp in self.results:
            ...
            company_id = temp[1]
            fiscal_year_list = []
            sql_jud = 'select fiscal_year from financial_statement_index where country_code = "IND" and exchange_market_code = "BSE" and doc_type = "pdf" and financial_statement_season_type_code = "FY" and start_date is null and end_date is null and company_code = %s'
            self.cursor.execute(sql_jud, temp[1])
            results = self.cursor.fetchall()
            for temp in results:
                if temp[0] not in fiscal_year_list:
                    fiscal_year_list.append(temp[0])
                    fiscal_year_list.sort()
            if len(fiscal_year_list) != 0:
                max_fiscal_year = fiscal_year_list[-1]
