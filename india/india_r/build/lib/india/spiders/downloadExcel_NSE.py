...
class CompanylistSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for temp in self.results:
            ...
            report_id_list = []
            sql_jud = 'select report_id from financial_statement_index where country_code = "IND" and exchange_market_code = "NSE" and doc_type = "csv" and company_code = %s'
        ...
        if id not in report_id_list:
            ...
            for temp in tr_list:
                ...
                if title is not None or title != "\n" or title != " " or title != " ":
                    with open("D:\item\OPDCMS/report data update\india\data\csv/" + item["report_id"] + ".csv", "a") as f:
                        f.write(title + "," + value + "\n")
