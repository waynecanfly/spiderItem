...
class indiaExcuteThird(scrapy.Spider):
    name = "indiaExcuteThird"
    ...
    def get_bse_code_list(self):
        """获得BSE最新的code list"""
        global flag, excel_name_bse
        newest_name_num = self.get_newest_company_file("BSE")
        excel_name_bse = "/data/OPDCMS/india/listed_company_update/company_list/BSE_" + newest_name_num + ".csv"
        with open(excel_name_bse, "r") as f:
            ...
            for temp in data:
                if self.flag:
                    code = temp.split(",")[0]
                    symbol = temp.split(",")[1]
                    if code not in self.code_list_bse:
                        self.code_list_bse.append(code)
                    if symbol not in self.symbol_list_bse:
                        self.symbol_list_bse.append(symbol)
                self.flag = True
            self.flag = None

    def get_nse_code_list(self):
        """获得NSE最新的code list"""
        global flag, excel_name_nse
        newest_name_num = self.get_newest_company_file("NSE")
        excel_name_nse = "/data/OPDCMS/india/listed_company_update/company_list/NSE_" + newest_name_num + ".csv"
        with open(excel_name_nse, "r") as f:
            data = f.readlines()
            for temp in data:
                if self.flag:
                    symbol = temp.split(",")[0].replace('"', "")
                    if symbol not in self.symbol_list_nse:
                        self.symbol_list_nse.append(symbol)
                else:
                    self.flag = True
            self.flag = None
    ...
    def get_all_code_list(self):
        ...
        self.get_bse_code_list()
        self.get_nse_code_list()
        for temp in self.symbol_list_nse:
            if temp not in self.symbol_list_bse:
                self.go_heavy_symbol_list.append(temp)
    ...
    def get_not_have_code_list(self):
        ...
        for temp in results:
            code = temp[0]
            company_id = int(str(temp[1]).replace("IND", ""))
            ...
            self.symbol_list_in_mysql.append(info_disclosure_id)
        for each in self.code_list_bse:
            if each not in self.code_list_in_mysql:
                self.new_code_list.append(each)
        for eachOne in self.go_heavy_symbol_list:
            if eachOne not in self.symbol_list_in_mysql:
                self.new_symbol_list.append(eachOne)
        max_company_num = self.company_num_list[-1]

    def insert_new_data_to_mysql(self, sList, param, name):
        ...
        for temp in data:
            ...
            if code in sList:
                if name == "bse":
                    ...
                    tag = "security_code"
                max_company_num += 1
                company_id = "IND" + str(max_company_num)
                ...
                self.cursor.execute(sql, [company_id, code, tVlaue, 0, company_name, download_link, spiderName])
                self.conn.commit()
                self.num += 1
                print(self.num)
    ...
    def start_requests(self):
        ...
        self.insert_new_data_to_mysql(self.new_code_list, "security_code", "bse")
        self.insert_new_data_to_mysql(self.new_symbol_list, "info_disclosure_id", "nse")
        url = "https://www.baidu.com/"
        yield scrapy.Request(url, callback=self.parse)

    def parse(self, response):
        print("The third part is complete!!!")
