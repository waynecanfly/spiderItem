...
class CompanylistSpider(scrapy.Spider):
    ...
    def saveExcel(self, tr_list, name):
        for temp in tr_list:
            ...
            if title is not None:
                with open("D:\item\OPDCMS\listed company update\india\data\csv/" + name + ".csv", "a") as f:
                    f.write(title + "," + value + "\n")
    ...
    def parse(self, response):
        ...
        for temp in url_list:
            yield scrapy.Request(temp, callback=self.download, meta={"code": code, "company_id": company_id, "type": "Q"})
        url_list2 = self.judgument_func(annual_list)
        for each in url_list2:
            yield scrapy.Request(each, callback=self.download, meta={"code": code, "company_id": company_id, "type": "FY"})
