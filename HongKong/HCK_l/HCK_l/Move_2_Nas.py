...
class Move2Nas(object):
    ...
    def get_fiscal_year(self, file_name):
        ...
        if result:
            sql = "update non_financial_statement_index set is_downloaded = 1, gmt_update=%s where report_id=%s"
            self.cursor.execute(sql, [time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())), file_name.split(".")[0]])
            self.conn.commit()
            return str(result[0])
        else:
            ...
            result = self.cursor.fetchone()
            if result:
                sql = "update financial_statement_index set is_downloaded = 1, gmt_update=%s where report_id=%s"
                self.cursor.execute(sql, [time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())), file_name.split(".")[0]])
                self.conn.commit()
                return str(result[0])
            else:
    ...
    def Move2NasMain(self, LocalDir, NasDir):
        ftp = self.ftpconnect("10.100.4.102", "root", "originp123")
