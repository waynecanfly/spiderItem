...
class Move2Nas(object):
    ...
    def Move2NasMain(self):
        ...
        dir_list = os.listdir("/data/OPDCMS/HCK/pdf_l")
        for temp in dir_list:
            fiscal_year = self.get_fiscal_year(temp)
            try:
                ftp.mkd("/homes3/HongKong/" + fiscal_year)
            except:
                pass
            self.num += 1
            self.uploadfile(ftp, "/homes3/HongKong/" + fiscal_year + "/" + temp, "/data/OPDCMS/HCK/pdf_l/" + temp)
