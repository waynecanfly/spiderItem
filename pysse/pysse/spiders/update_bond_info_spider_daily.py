...
class UpdateBondInfoDailySpider(scrapy.Spider):
    name = 'update_bond_info_daily'
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(UpdateBondInfoDailySpider, cls).from_crawler(
            crawler, *args, **kwargs
        )
    ...
    def spider_opened(self, spider):
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                """\
                    select * from bond_of_china where user_create='lq' order \
                    by id desc limit 20\
                """
            )
